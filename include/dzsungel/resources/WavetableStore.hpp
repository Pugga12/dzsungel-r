...
constexpr size_t kDefaultLength = 4096;
namespace dzsungel::resources {
    class WavetableStore {
    public:
        const Wavetable* generateSine(std::string_view id, size_t length = kDefaultLength);
        const Wavetable* generateTriangle(std::string_view id, size_t length = kDefaultLength);
